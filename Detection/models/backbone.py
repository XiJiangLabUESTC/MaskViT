...
class VisionTransformer(nn.Module):
    ...
    def forward_features_with_mask(self,x,mask_rate,mask_layer,mask=None):
        ...
        cls_token_nums, det_token_num = cls_tokens.shape[1],det_token.shape[1]
        ...
        cls_tokens = x[:,:cls_token_nums,:]
        det_token = x[:,-det_token_num:,:]
        x = x[:,cls_token_nums:-det_token_num,:]
        ...
        x_ = torch.cat((cls_tokens, x_, det_token), dim=1)
        
        for i in range(mask_layer):
            if self.use_checkpoint:
                x_ = checkpoint.checkpoint(self.blocks[i], x_)    # saves mem, takes time
            else:
                x_ = self.blocks[i](x_)
        cls_tokens = x_[:,:cls_token_nums,:]
        det_token = x_[:,-det_token_num:,:]
        x_ = x_[:,cls_token_nums:-det_token_num,:]
...
if __name__ == '__main__':
    model,hidden_dim = base()
    img = torch.randn((4,3,384,384))
    model.finetune_det()
    out = model.forward_features_with_mask(img,mask_rate=0.4,mask_layer=8,mask=None)
    print(out.shape)
